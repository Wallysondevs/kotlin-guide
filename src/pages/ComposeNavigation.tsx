export default function ComposeNavigation() {
  return (
    <article className="chapter max-w-3xl mx-auto px-6 py-10">
      <div className="text-xs text-muted-foreground mb-2 font-mono">Android · intermediario · 9 min</div>
      <h1>Navigation Compose</h1>
      <div dangerouslySetInnerHTML={{__html: `<p><strong>Navigation Compose</strong> é a biblioteca oficial AndroidX para navegação entre <em>composables</em>. Em vez de <code>Fragment</code>s e <code>FragmentManager</code>, você define um grafo declarativo de rotas — cada rota mapeia para um <em>composable</em>.</p>
...
<div class="callout callout-tip"><div class="callout-title">Animações</div><div>Use <code>composable&lt;X&gt;(enterTransition = { slideInHorizontally() }, exitTransition = { fadeOut() })</code> para transições por destino sem precisar de <code>AnimatedNavHost</code> de terceiros.</div></div>`}} />
    </article>
  );
}
